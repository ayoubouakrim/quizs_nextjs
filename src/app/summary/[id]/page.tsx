...
import { ParagrapheService } from '@/services/ParagrapheService';
...
const SummaryPage = ({ params }: Props) => {
    ...
    const [paragraphes, setParagraphs] = useState<Paragraphe[]>([]);
    const [summary, setSummary] = useState<Summary | null>(null);
    const [currentParagraph, setCurrentParagraph] = useState(0);
    const [readingMode, setReadingMode] = useState<'single' | 'all'>('all');
    const [isBookmarked, setIsBookmarked] = useState(false);
    const [expandedCards, setExpandedCards] = useState<Set<string>>(new Set());

    // Services
    const summaryService = new SummaryService();
    const paragrapheService = new ParagrapheService();

    // Fetch data from backend
    const getSummaryWithParagraphs = async (summaryId: string) => {
        try {
            setLoading(true);
            
            // Fetch summary and paragraphs separately as you're doing
            const [summaryData, paragraphsData] = await Promise.all([
                summaryService.getSummaryById(summaryId),
                paragrapheService.getParagraphsBySummaryId(summaryId)
            ]);
            
            setSummary(summaryData);
            setParagraphs(paragraphsData);
        } catch (error) {
            console.error("Error fetching summary and paragraphs:", error);
            setSummary(null);
            setParagraphs([]);
        } finally {
            setLoading(false);
    ...
    useEffect(() => {
        getSummaryWithParagraphs(summaryId);
    }, [summaryId]);
    ...
    const goToNextParagraph = () => {
        if (currentParagraph < paragraphes.length - 1) {
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                            <button
                                ...
                                className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-200 ${readingMode === 'single'
                            ...
                            <button
                                ...
                                className={`p-2 rounded-full transition-all duration-200 ${isBookmarked ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'
                                    }`}
                            >
                ...
                <div className="max-w-6xl mx-auto px-6 pt-4">
                    ...
                    <div className="mb-8">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                            ...
                            <div className="flex items-center space-x-1">
                                ...
                                <span>{Math.ceil(paragraphes.length * 1.5)} min read</span>
                            </div>
                            <div className="flex items-center space-x-1">
                    ...
                    {paragraphes.length === 0 ? (
                        <div className="bg-white rounded-lg p-8 shadow-sm text-center">
                            ...
                            <p className="text-gray-600 text-lg">No paragraphs found for this summary.</p>
                        </div>
                    ) : readingMode === 'single' ? (
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded-lg p-4 shadow-sm">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-sm font-medium text-gray-700">
                                        Section {currentParagraph + 1} of {paragraphes.length}
                                    </span>
                                    <span className="text-sm text-gray-500">
                                        {Math.round(((currentParagraph + 1) / paragraphes.length) * 100)}% complete
                                    </span>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div
                                        className="bg-gradient-to-r from-indigo-500 to-purple-500 h-2 rounded-full transition-all duration-300"
                                        style={{ width: `${((currentParagraph + 1) / paragraphes.length) * 100}%` }}
                                    />
                                </div>
                            </div>
                                ...
                                <div className="flex items-start space-x-4">
                                    ...
                                    <div className="flex-1">
                                        <h3 className="text-xl font-semibold text-gray-900 mb-3">
                                            {paragraphes[currentParagraph].title}
                                        </h3>
                                        <p className="text-gray-700 leading-relaxed">
                                            {paragraphes[currentParagraph].content}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between bg-white rounded-lg p-4 shadow-sm">
                                <button
                                    ...
                                    className={`flex items-center space-x-2 px-4 py-2 rounded-full transition-all duration-200 ${currentParagraph === 0
                                ...
                                <div className="flex items-center space-x-2">
                                    {paragraphes.map((_, index) => (
                                        <button
                                            key={index}
                                            onClick={() => setCurrentParagraph(index)}
                                            className={`w-3 h-3 rounded-full transition-all duration-200 ${index === currentParagraph
                                                    ? 'bg-indigo-500'
                                ...
                                <button
                                    ...
                                    disabled={currentParagraph === paragraphes.length - 1}
                                    className={`flex items-center space-x-2 px-4 py-2 rounded-full transition-all duration-200 ${currentParagraph === paragraphes.length - 1
                                            ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                            : 'bg-indigo-100 text-indigo-600 hover:bg-indigo-200'
                                        }`}
                                >
